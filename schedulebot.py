...
#should this be here? nope, but im not making a new file for it. :)
#also there is absolutely a more clever and efficient way to iterate all of this, but it's good enough for now

# this is literally hell to read because i havent commented any of it, but it works (for now)
async def makeEmbeds(title, description, unformatted_data):
    ...
    for i in range(len(body_list)):
        ...
        if i == 0:
            current_embed = discord.Embed(title=title, description=description, color=embed_color)
        else:
            current_embed = discord.Embed(title='Continued', description='\n', color=embed_color)
        for line in body.splitlines():
            ...
            if r != None:
                if current_title == '' and current_value == '':
                    ...
                    current_embed.add_field(name=f'{name} Continued', value=current_value, inline=False)
                    current_value = ''
                else:
                    current_embed.add_field(name=current_title, value=current_value, inline=False)
                    current_value = ''
                current_title = r.groups()[0]
            else:
                current_value += f'{line}\n'
        embeds_made = b64.urlsafe_b64encode(embed_handler.token.encode()).decode('utf-8')
        if current_title == '' and current_value != '':
            last_embed = embed_list[i - 1]
            name = last_embed.fields[-1].name
...
async def on_message(message):
    if message.author == client.user:
        pass
    elif ':catuwu:' in message.content:
        catuwu = client.get_emoji(841142074593378344)
...
async def checkschedule(ctx, studentid = 'optional'):
    ...
    try:
        ...
        response_embeds = await makeEmbeds(title=f'"{student_name}" Sections', description=f'List of classes and sections of "{student_name}" and other students taking the same ones.', unformatted_data=unformatted_response)
        requests.get(f"http://{scheduling.campus_active}{discord_url}/{embeds_made}")
    except FileNotFoundError:
        await ctx.send('No entries found in my database. Please upload your schedule; try: \'+help uploadschedule\'. If you have already uploaded your schedule but are now having issues, run \'+help reassociate\'')
        mongo.closeConnection()
        return
...
async def checkclass(ctx, *, courseid = 'append'):
    ...
    try:
        ...
        response_embeds = await makeEmbeds(title=f'Students taking {courseid}', description='', unformatted_data=unformatted_response)
    except SyntaxError:
...
async def checkprof(ctx, *, courseid = 'append'):
    ...
    current_embed = discord.Embed(title=f'Professors for {courseid}', description='Enter a number to choose.', color=0x00529B)
    professorsString = ''
    i = 0
    check_exists = 0
    ...
    response_embeds = await makeEmbeds(title=f'Students with {prof} for {courseid}', description='', unformatted_data=unformatted_response)
