...
class MongoManage:
    ...
    def findSimilarSection(self, lookup_index, lookup_type):
        ...
        for class_dict in class_section_dicts:
            found_names = self.schedules.find({'classes': {'course': class_dict['course'], 'section': class_dict['section'], 'instructor': class_dict['instructor']}},{'name': 1, 'username': 1, 'saved_nickname': 1})
            names_list = []
            for student in found_names:
                if student['name'] == canonical_name or student == None:
                    pass
    ...
    def findStudentswithProfessor(self, prof):
        ...
        found_names = self.schedules.find({'classes.instructor': prof}, {'name': 1, 'username': 1, 'saved_nickname': 1})
        names_list = []
        output_dict = {}
